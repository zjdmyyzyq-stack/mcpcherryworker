// 🦞 Cloudflare MCP Worker - 数据库式规则/免配置使用
// 使用 Cloudflare KV 存储
// ===================== 配置说明 =====================
// 部署前需在 Cloudflare 面板：
// 1. 创建 KV 命名空间 → 取名 "MCP_DATA"
// 2. Worker 设置 → 绑定 KV 命名空间
// 3. 变量名：MCP_DATA
// 4. 选择刚才创建的 "MCP_DATA" 命名空间
// ====================================================

// In service worker format, KV bindings are global variables
// MCP_DATA is available as a global variable
...
async function handleRequest(request) {
  ...
  if (request.method === 'GET' && url.pathname === '/') {
    let ruleCount = 0;
    try {
      const keys = await MCP_DATA.list();
    ...
    return new Response(`
      ...
      <body>
        ...
        <div class="stat">🗳️ 存储类型: KV 数据库</div>
        <div class="stat">📝 规则数量: ${ruleCount}</div>
...
async function handleMCP(body) {
  ...
  if (method === 'tools/call') {
    ...
    try {
      switch (toolName) {
        case 'get_rule': {
          const value = await MCP_DATA.get(args.key);
          return {
            jsonrpc: '2.0',
            id,
            result: {
        ...
        case 'set_rule': {
          await MCP_DATA.put(args.key, args.value);
          return {
            jsonrpc: '2.0',
            id,
            result: {
        ...
        case 'list_rules': {
          const list = await MCP_DATA.list();
        ...
        case 'delete_rule': {
          await MCP_DATA.delete(args.key);
          return {
            jsonrpc: '2.0',
            id,
            result: {
        ...
        case 'search_rules': {
          const all = await MCP_DATA.list();
          const results = [];
          for (const key of all.keys) {
            const val = await MCP_DATA.get(key.name);
